class ChromecastSlideshowController {
    ...
    setupSocketEventListeners() {
        this.socket.on('connect', async () => {
            ...
            // Sync with current playlist status on connect (but don't emit WebSocket to avoid conflicts)
        ...
        // Removed duplicate slideshow handlers - using playlist-specific handlers below
        ...
        // Removed playlist_started and playlist_stopped handlers - using playlist_status_update for everything
        ...
        this.socket.on('playlist_status_update', (status) => {
            console.log('📡 SKIP DEBUG: WebSocket playlist_status_update received:', {
                running: status.running, 
                current_item_id: status.current_item?.id, 
                current_item_name: status.current_item?.directory_name
            });
        ...
        // Removed old single-directory slideshow handlers - using playlist system only
    }
        ...
        try {
            ...
            if (playlistStatus.running) {
                ...
                if (!response.ok) {
                    ...
                    // Force update status after skip to ensure frontend syncs even if WebSocket fails
    ...
    updateSlideshowControls(isRunning, mode = 'playlist') {
        ...
        this.startPlaylistBtn.disabled = isRunning || this.playlistItems.length === 0;
        ...
        if (this.slideshowRunningEl) {
            console.log('🎮 Current text before update:', this.slideshowRunningEl.textContent);
            // Always show playlist status since we're primarily using playlists now
            this.slideshowRunningEl.textContent = isRunning ? 'Running' : 'Stopped';
            this.slideshowRunningEl.style.color = isRunning ? '#27ae60' : '#e74c3c';
            console.log('🎮 Text after update:', this.slideshowRunningEl.textContent);
        } else {
    ...
    updatePlaylistButtonStates() {
        ...
        this.startPlaylistBtn.disabled = !hasValidItems || !hasEnabledDevices;
        this.clearPlaylistBtn.disabled = this.playlistItems.length === 0;
    }
    
    highlightCurrentPlaylistItem(status) {
        console.log('🎯 SKIP DEBUG: highlightCurrentPlaylistItem called with:', {
            running: status.running,
            current_item_id: status.current_item?.id,
            current_item_name: status.current_item?.directory_name
        });
        
        // Remove arrows from all playlist item names
        const allItems = this.playlistListEl.querySelectorAll('.playlist-item');
        allItems.forEach(item => {
            const nameEl = item.querySelector('.playlist-item-name');
            if (nameEl && nameEl.textContent.startsWith('▶️ ')) {
                nameEl.textContent = nameEl.textContent.substring(3); // Remove "▶️ "
            }
            item.classList.remove('currently-playing');
        });
        
        // Add arrow to current item name if playlist is running
        if (status.running && status.current_item) {
            ...
            if (currentItemElement) {
                const nameEl = currentItemElement.querySelector('.playlist-item-name');
                if (nameEl && !nameEl.textContent.startsWith('▶️ ')) {
                    nameEl.textContent = '▶️ ' + nameEl.textContent;
                    console.log('🎯 SKIP DEBUG: Added arrow to playlist item name:', nameEl.textContent);
                }
                currentItemElement.classList.add('currently-playing');
            } else {
                console.log('🎯 SKIP DEBUG: Could not find element with data-item-id:', status.current_item.id);
            }
        } else {
            console.log('🎯 SKIP DEBUG: Not adding arrow - running:', status.running, 'has current_item:', !!status.current_item);
        }
    }
